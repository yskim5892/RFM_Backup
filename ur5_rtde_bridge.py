...
class UR5RTDEBridge(Node):
    def __init__(self):
        ...
        self.sub_target = self.create_subscription(
            PoseStamped, "/pose_tracker/target_tcp_pose", self._on_target, 10
        )

        self.sub_relation_target = self.create_subscription(
            PoseStamped, "/pose_tracker/target_tcp_pose_r", self._on_relation_target, 10
        )
        ...
        self.get_logger().info(
            f"UR5 RTDE bridge connected to {self.robot_ip}. Subscribing /pose_tracker/target_tcp_pose"
        )
